    ...
    class LinkModel {
        ...
        public function addLink($authorID, $longURL, $token) {
            if($longURL == '')
                ...
                return 'Поле ссылки не может быть меньше 10 символов';
            else if($longURL != filter_var($longURL, FILTER_SANITIZE_URL))
                return 'Поле ссылки может содержать только буквы, цифры и допустимые символы $-_.+!*\'(),{}|\\^~[]`<>#%";/?:@&=.';
            else if(strlen($token) < 3)
                return 'Поле сокращенного названия не может быть меньше 3 символов';
            else if(strlen($token) > 70)
                return 'Поле сокращенного названия не может быть больше 70 символов';
            else if(preg_match("/[^а-яА-ЯёЁa-zA-Z0-9\-_\/]+$/u", $token))
                return 'Поле сокращенного названия не должно содержать любые символы, кроме: а-я, А-Я, a-z, A-Z, 0-9, /, -, _)';
